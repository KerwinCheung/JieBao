...
@interface ChannelSettingViewController ()<GizWifiDeviceDelegate>
...
{
    if (btn.selected) {
        [self alertShowMessage:@"您已经开启定时，若需手动控制请先关闭定时" title:@"提示" confirmBtnText:@"确定" confirmCallback:nil];
    }
    btn.selected = !btn.selected;
    [self.dev write:@{@"TimerON":@(btn.selected)} withSN:100];
}

- (void)tranferImgBtnClicked
{
    ChannelDetailViewController *vc = [ChannelDetailViewController new];
    vc.dev = self.dev;
    vc.title = self.title;
...
- (void)startSwitchChange
{
    ...
    [self.dev write:@{@"TimerON":@(self.turnBtn.selected)} withSN:100];
}
...
- (void)device:(GizWifiDevice *)device didReceiveData:(NSError *)result data:(NSDictionary<NSString *,id> *)dataMap withSN:(NSNumber *)sn
{
    if (result.code == GIZ_SDK_SUCCESS) {
        if ([sn integerValue] == 100) {
            [self alertShowMessage:@"定时开启成功" title:@"提示" confirmBtnText:@"确定" confirmCallback:nil];
        }
    }else
    {
        [self alertShowMessage:@"定时开启失败" title:@"提示" confirmBtnText:@"确定" confirmCallback:nil];
    }
}
- (UILabel *)subLb
{
