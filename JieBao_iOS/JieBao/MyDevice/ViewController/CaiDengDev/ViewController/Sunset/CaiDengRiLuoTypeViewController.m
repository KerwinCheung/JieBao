...
@interface CaiDengRiLuoTypeViewController ()<GizWifiDeviceDelegate>
...
@implementation CaiDengRiLuoTypeViewController
    ...
    if (self.dev) {
        ...
        for (CustomDevice *customDev in self.group.devs) {
            if ([SDKHELPER.statusDic.allKeys containsObject:customDev.did]) {
                ...
                self.slider.value = lightStatusModel.M4Num.floatValue;
...
- (void)settingBtnClicked
{
    ...
    if (self.dev) {
        [self.dev write:@{@"M4":@(sliderValue),@"Timer":@NO,@"mode":@(4)} withSN:104];
    }else
    {
        NSDictionary *body = @{@"attrs":@{@"M4":@(sliderValue),@"Timer":@NO,@"mode":@(4)}};
...
- (UIImageView *)nightImgView
{
    if (!_nightImgView) {
        ...
        _nightImgView.image = [UIImage imageNamed:@"pic4"];
    }
