...
@implementation CaiDengMorningTypeViewController
...
{
    if (self.dev) {
        [self.dev write:@{@"M1":@(self.slider.value),@"Timer":@NO,@"mode":@(1)} withSN:101];
    }else
    {
        NSDictionary *body = @{@"attrs":@{@"M1":@(self.slider.value),@"Timer":@NO,@"mode":@(1)}};
...
- (UIImageView *)nightImgView
{
    if (!_nightImgView) {
        ...
        _nightImgView.image = [UIImage imageNamed:@"pic1"];
    }
